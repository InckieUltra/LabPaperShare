...
public interface MySearpaper {
    @Select("select paper_id,title,conference,summary,name,type,field from (select paper_id,title,conference,summary,group_concat(distinct author_name) as name,type,group_concat(distinct field_name) as field from paper natural join publish natural join cover natural join field group by paper_id) as T where paper_id like '%${paper_id}%'")
